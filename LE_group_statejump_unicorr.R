# For the specified k and cognitive variable, find the correlations with
# the transition distance metrics and the corresponding uncorrected p-values.
# Output:
...
unipath <- paste0(basepath,'unicorr/',ccog,'/')

#Set parameters.
nk <- as.numeric(k)

#Get predictor data.
infile <- paste0(basepath,'transmean.csv')
predmat <- read_csv(infile,col_names=T) %>% 
  rename_at(1,~'Subject') 
colnames(predmat) <- gsub('-','t',c(colnames(predmat)[1],
                       paste0('jump_',colnames(predmat)[2:ncol(predmat)]))) 
nmet <- ncol(predmat) - 1
metlabs <- colnames(predmat[2:(nmet+1)])
...
#Set up matrix.
corrmat <- matrix(NA,nrow=nmet,ncol=ncog)
rownames(corrmat) <- metlabs
...
for (midx in 1:nmet) {
  ...
  corrmat[xmet,ycog] <- cor(data_Y[,ycog],data_X[,xmet],method='spearman')
  p_corrmat[xmet,ycog] <- cor.test(data_Y[,ycog],data_X[,xmet],method='spearman')$p.value
}
