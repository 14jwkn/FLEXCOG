%{
For the subject group and the specified k, extract the dynamics, transition
...
function [] = Step25_r_LE_group_staterecon_unicorr_FDR(subgroup,k)
...
basepath = append('../outputs/r_stateflex/statecalc_test/LE/ver_MATLAB/group/',...
                  subgroup,'/',k,'/');
dynpath = append(basepath,'dynstate/unicorr/');
simpath = append(basepath,'statesim/unicorr/');
jumppath = append(basepath,'statejump/unicorr/');
outpath = append(basepath,'allreconfig/unicorr/');
