%{
Read in the subject file for all subjects. For the LE(t) for each run and 
each participant, stack them and conduct k-medians clustering with a 
specified k for one random iteration of 500. 
Output:
...
function [] = Step11_r_LE_group_statecalc(subfile,k,iteration)
%subfile = Subject list file name.
%k = Selected k for k-medians clustering.
%iteration = Current random iteration of 500.
disp(append('Doing ',subfile,' ',k,' ',iteration));
tic
