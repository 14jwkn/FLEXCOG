...
# -*- coding: utf-8 -*-
"""
Read in all participants with ICA-FIX 3T resting-state data, then isolates 
...
with higher than 0.2 in any run. Find the intersection across subjects and output
the full list for analysis and a half list to examine parameters.
Output:
r_full_submain.txt Main list of subject IDs in each line.
r_half_submain.txt Random half list of subject IDs in each line.
...
random.seed(12345)
nfull = len(complist)
randlist = random.sample(complist,nfull)
subpart1 = randlist[:nfull//2]
subpart1.sort()
np.savetxt('r_half_submain.txt',subpart1,fmt='%s',delimiter="\n") 
