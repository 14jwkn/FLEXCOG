...
# the dynamics metrics and the cognitive variables and the corresponding uncorrected
# p-values.
# Output:
...
basepath <- paste0('../outputs/r_stateflex/statecalc_test/LE/ver_MATLAB/group/',
                   subgroup,'/',k,'/')
unipath <- paste0(basepath,'dynstate/unicorr/')
...
for (cidx in 1:ncog) {
  ...
  for (sidx in 1:nstate) {
    ...
    corrmat[xstate,ycog] <- cor(data_Y[,ycog],data_X[,xstate])
    p_corrmat[xstate,ycog] <- cor.test(data_Y[,ycog],data_X[,xstate])$p.value
  }
}
